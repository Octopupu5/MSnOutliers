...
namespace CP {
    namespace ML {
		...
		void Forest::Fit(const Common::Matrix &data) {
			Clear();
		...
		Node* Forest::CreateTree(const Common::Matrix& availableData, size_t depth) {
			...
			std::vector<std::vector<Common::Feature>> leftSplit;
			std::vector<std::vector<Common::Feature>> rightSplit;
			for (int j = 0; j < availableData.Rows(); ++j) {
		...
		uint32_t Forest::PathLength(Node* const tree, const Common::Matrix& sample) {
			uint32_t depth = 0.0;
		...
		// 1 = anomaly / 0 = inlier
		double Forest::PredictProba(const Common::Matrix& sample) {
